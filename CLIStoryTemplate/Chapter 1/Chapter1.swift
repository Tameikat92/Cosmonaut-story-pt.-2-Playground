   ...
   """
	...
	func performTaskC() {
		...
		func characterEncountersMarauders(CosmonautName: String) {
			...
			print("Oh no \(CosmonautName) ! It looks like some \((villians[0])) have gotten in the villiage pretending to be weak travelers in need of our help. They may try to kidnap some more of our people and steal our herbs and livestock.")
			...
			if let userInput = readLine(), userInput.uppercased() == "Yes" {
				...
				if isDangerous {
					print("The (villians[0]) are stealing our livestock and herbs! \(characters[1]) must fight for their life. They are trying to kidnap our people from the village and force them into submission.")
				} else {
				...
				let isDangerous = Bool.random()
				if isDangerous {
					print("It turns out \(villians[0]) are dangerous. They are stealing our livestock and herbs! \(characters[1]) must fight like Achilles in Troy!.")
					...
					func main() {
						print("\(CosmonautName), welcome to The battle of \(characters[1]) and \(villians[0])!")
						...
						print("You encounter a group of \(villians[0]) in the woods. What will you do?")
						print("1. Fight")
						print("2. Flee")
